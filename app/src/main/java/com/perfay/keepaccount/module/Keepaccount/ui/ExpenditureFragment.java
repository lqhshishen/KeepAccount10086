...
public class ExpenditureFragment extends BaseFragment<ExpenditurePresenter> implements ExpenditureContract.view {
    ...
    @Override
    public void setPresenter(ExpenditurePresenter presenter) {
        if (this.presenter == null)
            this.presenter = new ExpenditurePresenter();
    }
...
//                ToastUtils.showShort(getContext(),"你点击了" + presenter.getClassify().get(pos).getName());
    ...
    public void judge() {
        ...
        presenter.addInf(time,useType,money * -1,account,beizhu);
        ToastUtils.showShort(getContext(),"录入成功");

        EventBus.getDefault().post(new KeepAccountDatabase(time,useType,money * -1,account));
        getActivity().finish();
    }
